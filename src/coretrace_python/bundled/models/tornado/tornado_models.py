...
# What ``self.request`` gives as text; its body and files may hold a structure.
_REQUEST_TEXT = (
    "arguments", "query_arguments", "body_arguments", "query", "uri", "path", "headers", "cookies",
    "method", "host", "host_name", "remote_ip", "protocol", "version", "full_url",
)
...
class TornadoModels(ModelPlugin):
    ...
    models: ClassVar[tuple[Model, ...]] = (
        ...
        *(RequestObject(_sym(f"{handler}.request"), _REQUEST_TEXT) for handler in _HANDLERS),
